...
class PascalDataSet(data.Dataset):
    def __init__(
        ...
        data_list_path = os.path.join(data_root, split+".txt")
        ...
        #         name = "/".join(name_split[2:])
            ...
            self.data_list.append(
                {
                    "img": os.path.join(self.data_root,"JPEGImages" , "%s.jpg" % fname),
                    "label": os.path.join(self.data_root, 'SegmentationClass', "%s.png" % fname),
                    "name": name,
                }
            )
        print("Loaded DRAM {} set with {} images".format(self.split, len(self.data_list)))
